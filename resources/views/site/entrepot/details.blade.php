...
<div class="row">
    <div class="col-md-8 reveal">
        <div class="single-property">
            <div class="widget">
                <h2 style="font-family: 'Manjari Regular'" class="widgettitle">Details d'un entrepot </h2>
            </div>
            <h3 class="page-title">{{ $entrepot->name }}</h3>
            <div class="property-slider">
                <div id="property-images" class="flexslider">
                    <ul class="slides">
                        ...
                        @endforeach
                        <li class="item"> <img src="{{URL::to($entrepot['image'])}}" alt=""> </li>
                    </ul>
                </div>
    ...
    <div class="sidebar right-sidebar col-md-4 reveal-2">
        <div class="widget">
            <div class="agent">
                <h4><i style="color: red" class="fa fa-map-marker"></i> {{ $entrepot->adresse }}</h4>
                <p>{{ $entrepot->short_description }}</p>
                <div class="agent-contacts clearfix">
                    @if($entrepot->devise == 1)
                    <div style="background-color: red;" class="price"><span>{{ $entrepot->prix }}</span><strong>CFA</strong></div>
                    @elseif($entrepot->devise == 2)
                    <div class="price"><span>{{ $entrepot->prix }}</span><strong>EURO</strong></div>
                    @else
                    <div class="price"><span>{{ $entrepot->prix }}</span><strong>$</strong></div>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="container">
            ...
            <div class="row">
                <ul class="owl-carousel owl-alt-controls" data-columns="4" data-autoplay="yes"
                    ...
                    @foreach($entrepot_similaire as $entrepot)
                    <li class="item property-block">
                        <a href="{{ route('detail_entr', array('select' => $entrepot->id)) }}" class="property-featured-image">
                            <img style="height: 150px;"  src="{{ asset($entrepot->image) }}" alt="">
                            @if($entrepot->option == 1)
                        ...
                        <div class="property-info">
                            <h4><a href="{{ route('detail_entr', array('select' => $entrepot->id)) }}">{{ $entrepot->name }}</a></h4>
                            <span class="location">{{ $entrepot->adresse }}</span>
                            @if($entrepot->devise == 1)
                            <div style="background-color: rgba(2,72,255,0.76)" class="price"><span>{{ $entrepot->prix }}</span><strong>CFA</strong></div>
                            @elseif($appart->devise == 2)
                            <div style="background-color: rgba(2,72,255,0.76)" class="price"><span>{{ $entrepot->prix }}</span><strong>EURO</strong></div>
                            @else
                            <div style="background-color: rgba(2,72,255,0.76)" class="price"><span>{{ $entrepot->prix }}</span><strong>$</strong></div>
                            @endif
                        </div>
                    </li>
                    @endforeach
