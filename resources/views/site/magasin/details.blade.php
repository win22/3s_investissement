...
<div class="row">
    <div class="col-md-8 reveal">
        <div class="single-property">
            <div class="widget">
                <h2 style="font-family: 'Manjari Regular'" class="widgettitle">Details d'un magasin </h2>
            </div>
            <h3 class="page-title">{{ $magasin->name }}</h3>
            <div class="property-slider">
                <div id="property-images" class="flexslider">
                    <ul class="slides">
                        ...
                        @endforeach
                        <li class="item"> <img src="{{URL::to($magasin['image'])}}" alt=""> </li>
                    </ul>
                </div>
    ...
    <div class="sidebar right-sidebar col-md-4 reveal-2">
        <div class="widget">
            <div class="agent">
                <h4><i style="color: red" class="fa fa-map-marker"></i> {{ $magasin->adresse }}</h4>
                <p>{{ $magasin->short_description }}</p>
                <div class="agent-contacts clearfix">
                    @if($magasin->devise == 1)
                    <div style="background-color: red;" class="price"><span>{{ $magasin->prix }}</span><strong>CFA</strong></div>
                    @elseif($magasin->devise == 2)
                    <div class="price"><span>{{ $magasin->prix }}</span><strong>EURO</strong></div>
                    @else
                    <div class="price"><span>{{ $magasin->prix }}</span><strong>$</strong></div>
                    @endif
                </div>
            </div>
        </div>
...
<form class="forma reveal-3"  method="post" action="{{ route('save_mess', array('test' => $magasin->name)) }}">
    @csrf
    <div class="row">
        ...
        <div class="container">
            ...
            <div class="row">
                <ul class="owl-carousel owl-alt-controls" data-columns="4" data-autoplay="yes"
                    ...
                    @foreach($magasin_similaire as $magasin)
                    <li class="item property-block">
                        <a href="{{ route('detail_mag', array('select' => $magasin->id)) }}" class="property-featured-image">
                            <img style="height: 150px;"  src="{{ asset($magasin->image) }}" alt="">
                            @if($magasin->option == 1)
                        ...
                        <div class="property-info">
                            <h4><a href="{{ route('detail_mag', array('select' => $magasin->id)) }}">{{ $magasin->name }}</a></h4>
                            <span class="location">{{ $magasin->adresse }}</span>
                            @if($magasin->devise == 1)
                            <div style="background-color: rgba(2,72,255,0.76)" class="price"><span>{{ $magasin->prix }}</span><strong>CFA</strong></div>
                            @elseif($appart->devise == 2)
                            <div style="background-color: rgba(2,72,255,0.76)" class="price"><span>{{ $magasin->prix }}</span><strong>EURO</strong></div>
                            @else
                            <div style="background-color: rgba(2,72,255,0.76)" class="price"><span>{{ $magasin->prix }}</span><strong>$</strong></div>
                            @endif
                        </div>
                    </li>
                    @endforeach
